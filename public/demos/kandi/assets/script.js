(function() {
  ...
  function initHeroScene() {
    ...
    // Lighting — daylight style, warm and light
    var ambient = new THREE.AmbientLight(0xfff8f0, 0.7);
    scene.add(ambient);
    ...
    var fill = new THREE.DirectionalLight(0xe8d8c0, 0.4);
    ...
    // Frame — slightly larger, darker wood
    var frameGeom = new THREE.BoxGeometry(3.0, 4.4, 0.08);
    var frameMat = new THREE.MeshStandardMaterial({ color: 0x7a5830, roughness: 0.75, metalness: 0.05 });
    ...
    var panelMat = new THREE.MeshStandardMaterial({ color: 0xb08040, roughness: 0.65, metalness: 0.08 });
    var panel = new THREE.Mesh(panelGeom, panelMat);
    group.add(panel);

    // Inner recessed panel detail — lighter tone
    var innerGeom = new THREE.BoxGeometry(1.8, 3.2, 0.04);
    var innerMat = new THREE.MeshStandardMaterial({ color: 0xc89050, roughness: 0.7, metalness: 0.05 });
    ...
    var handleMat = new THREE.MeshStandardMaterial({ color: 0xc8a840, roughness: 0.25, metalness: 0.9 });
    ...
    // Handle end caps — bottom
    var capBot = new THREE.Mesh(capGeom, handleMat);
    capBot.position.set(0.9, -0.64, 0.15);
    group.add(capBot);

    // Position group slightly right so hero copy occupies left
  ...
  function initDoorScene() {
    ...
    if (!doorPanel || typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
  ...
  function initDrawerScene() {
    ...
    var fill = new THREE.DirectionalLight(0xe0d8c8, 0.35);
    fill.position.set(-3, 1, 3);
    scene.add(fill);

    // Cabinet body (fixed — behind drawer)
    var bodyGeom = new THREE.BoxGeometry(5.2, 1.4, 2.8);
    var bodyMat = new THREE.MeshStandardMaterial({ color: 0x8a6030, roughness: 0.8, metalness: 0.03 });
    ...
    var slotMat = new THREE.MeshStandardMaterial({ color: 0x5a3c18, roughness: 0.9 });
    ...
    // Front face — oak finish
    var frontGeom = new THREE.BoxGeometry(4.8, 1.0, 0.15);
    var frontMat = new THREE.MeshStandardMaterial({ color: 0xb08040, roughness: 0.65, metalness: 0.07 });
    ...
    var innerFrontMat = new THREE.MeshStandardMaterial({ color: 0xc89050, roughness: 0.7 });
    ...
    var hMat = new THREE.MeshStandardMaterial({ color: 0xc8a840, roughness: 0.25, metalness: 0.9 });
  ...
  function initCraftSection() {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: function(self) {
        ...
        // headline fades in 0 → 0.1
        if (headline) {
          headline.style.opacity = Math.min(1, p / 0.1);
        }
  ...
  function initMobileReveals() {
    // Show mobile fallback panels, hide desktop-only elements
    ...
    // All craft panels, finale, headline start visible on mobile
    ...
    if (craftHeadline) {
      craftHeadline.style.opacity = '1';
    }

    // Door reveal text visible on mobile
